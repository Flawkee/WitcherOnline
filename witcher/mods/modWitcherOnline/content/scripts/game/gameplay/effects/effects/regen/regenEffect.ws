abstract class W3RegenEffect extends CBaseGameplayEffect
{
    ...
    event OnUpdate(dt : float)
    {
        ...
        if(stat == BCS_Vitality && isOnPlayer && target == GetWitcherPlayer()
            && theGame.r_getMultiplayerClient().getDuelController().isActive())
        {
            canRegen = true;
        }
        else if(stat == BCS_Vitality && isOnPlayer && target == GetWitcherPlayer()
        ...
        if(canRegen)
        {
            ...
            if(regenPoints > 0)
            {
                if(stat == BCS_Vitality && isOnPlayer && target == GetWitcherPlayer()
                    && theGame.r_getMultiplayerClient().getDuelController().isActive())
                {
                    theGame.r_getMultiplayerClient().getDuelController().noteHealing(
                        regenPoints * dt, target.GetStatMax(stat));
                }
